...
%   matter.
%
% can use m20150603_DistributePPI_computation.m to run the batch in parallel.
...
for iSub = 1:numel(sublist)
    ...
    for iSess = 1:4
        ...
        % Use a customized version of spm_peb_ppi: the interaction term was
        % absurd for me (when low surp & low conf are negative, because 
        % they are centered, they result in a high value for the 
        % interaction term!!).
        PPI = spm_peb_ppi_fm(SPMname, 'phipi', VOI, [], ppiname, 0);
