...
NB: the output files from the MRI have horrible names. Keep a note of these horrible names, one session at a time, during the MRI acquisition. After the acquisition, rename the files so that a meaningful tag (e.g. sess1, sess2, etc.) appears in the filename. Then, fmspm12batch_RenameMove_PhysioFiles might help you to rename and move your files.
